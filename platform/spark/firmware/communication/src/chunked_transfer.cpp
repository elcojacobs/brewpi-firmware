...
ProtocolError ChunkedTransfer::handle_update_done(token_t token, Message& message, MessageChannel& channel)
{
    ...
    else
    {
        ...
        chunk_index_t increase = std::max(size_t(chunk_count*0.2), MINIMUM_CHUNK_INCREASE);	// ensure always some growth
        chunk_index_t resend_chunk_count = std::min(size_t(chunk_count+increase), MISSED_CHUNKS_TO_SEND);
        chunk_count = 0;
